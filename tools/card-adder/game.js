// game.js
// "Card Adder" - an addition drill. Reuses the playing-cards deck tool.
...
function startGame(rank) {
  ...
  els.compareSlot.innerHTML = renderCardHTML(state.compareCard);
  setPhase('playing');
  nextCard();
}
...
function renderRound() {
  els.currentSlot.innerHTML = renderCardHTML(state.currentCard);
...
function updateSum() {
  if (state.showAnswer && state.currentCard) {
    const total = state.compareCard.value + state.currentCard.value;
    els.sum.textContent = `${state.compareCard.value} + ${state.currentCard.value} = ${total}`;
  } else {
    els.sum.textContent = '+';
  }
}
